...
The panel's Export button downloads the effective settings as a `.env` file
that is line-compatible with the gameserver image's shipped `default.env` -
same key order, comments and section headers, for easy diffing.

The template belongs to the **gameserver**: at boot it copies its shipped
`default.env` to `<game volume>/default.env.template`, and the companion reads
it from there (override with `COMPANION_ENV_TEMPLATE`). This keeps the
companion image game-agnostic - any gameserver image can provide its own
template. When the file is absent the export still works, falling back to
schema order without the original comments.
